...
function Feed({ posts, getPosts, setPosts }) {
  ...
  const toggleLike = (post) => {
    if (post.isLiked) {
      axiosInst.delete(`/posts/${post.id}/like`)
      .then(() => {
      ...
      axiosInst.post(`/posts/${post.id}/like`)
      .then(() => {
        const updatedPosts = posts.map((item) => {
          if (item.id === post.id) {
            return {
  ...
  const toggleBookmark = (post) => {
    if (post.isBookmarked) {
      axiosInst.delete(`/posts/${post.id}/bookmark`)
      .then(() => {
        const updatedPosts = posts.map((item) => {
          if (item.id === post.id) {
            return {
      ...
      axiosInst.post(`/posts/${post.id}/bookmark`)
      .then(() => {
        const updatedPosts = posts.map((item) => {
          if (item.id === post.id) {
            return {
  ...
  const toggleRepost = (post) => {
    if (post.isRepostedByUser) {
      axiosInst.delete(`/posts/${post.id}/repost`)
      .then(() => {
        const updatedPosts = posts.map((item) => {
          if (item.id === post.id) {
            return {
      ...
      axiosInst.post(`/posts/${post.id}/repost`)
      .then(() => {
        const updatedPosts = posts.map((item) => {
          if (item.id === post.id) {
            return {
          ...
          <div className="flex gap-2">
            ...
            <div className="w-full">
              ...
              <div className="flex justify-between">
                ...
                <div className={post.isRepostedByUser ? "icon-value repost" : "icon-value"}>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
